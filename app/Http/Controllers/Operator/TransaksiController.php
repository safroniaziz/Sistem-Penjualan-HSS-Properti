...
class TransaksiController extends Controller
{
    public function index(){
        ...
        $transaksis = Transaksi::join('properti_details','properti_details.id','transaksis.properti_detail_id')
                                ...
                                ->select('transaksis.created_at','transaksis.id','nm_properti','jenis_properti','nm_konsumen','nik','pekerjaan','propertis.alamat','no_hp','booking','sisa_pembayaran','jenis_pembayaran')
                                ->get();
        return view('operator/transaksi.index',compact('properties','transaksis'));
    }
    ...
    public function post(Request $request){
        ...
        return redirect()->route('operator.transaksi')->with(['success'    =>  'Transaksi untuk "'.$properti->nm_properti.'" dengan nomor kavling "'.$properti->no_kavling.'" berhasil dilakukan !!']);
    }
    ...
    public function cetakLaporan($id){
        $data = Transaksi::join('properti_details','properti_details.id','transaksis.properti_detail_id')
                            ...
                            ->first();
                            // return $data;
        $pdf = PDF::loadview('operator.transaksi.laporan',[
            'data'  =>  $data
        ]);
